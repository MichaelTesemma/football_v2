...
for i in range(20):
    model = tf.keras.models.load_model(f"v2/saved_model10/ak_model_v10{i}",
    ...
    model.save(f"v2/ak_models_v10/ak_model_v10{i}", save_format="tf")
    print(f'model number {i} is saved')

for i in range(20):
    model = tf.keras.models.load_model(f"v2/ak_models_v10/ak_model_v10{i}",custom_objects=ak.CUSTOM_OBJECTS)
    ...
    with open('v2/ak_models_v10/stats.txt', 'a') as f:
