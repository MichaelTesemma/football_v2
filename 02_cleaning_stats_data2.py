...
fixtures_clean = pd.read_csv(f'v2/prem_clean_fixtures_and_dataframes/{fixtures_saved_name}')
# fixtures_clean = pd.read_csv(f'v2/prem_clean_fixtures_and_dataframes/filtered.csv')
...
for team in team_id_list:
    ...
    for j in team_fixture_list:
        #loading df
        df = pd.read_json('v2/prem_game_stats_json_files/' + str(j) + '.json', orient='values')
...
with open(f'v2/prem_clean_fixtures_and_dataframes/{stats_dict_output_name}', 'wb') as myFile:
    pickle.dump(all_stats_dict, myFile)

with open(f'v2/prem_clean_fixtures_and_dataframes/{stats_dict_output_name}', 'rb') as myFile:
